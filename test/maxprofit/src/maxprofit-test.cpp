...
class MaxProfitTest : public ::testing::Test {
  public:
    ...
    const vector<int> mixed_elements {13, -3, -25, 20, -3, -16, -23, 18, 20, -7, 12, -5, -22, 15, -4, 7};
    const vector<int> large_elements {numeric_limits<int>::max(), numeric_limits<int>::max()};
...
TEST_F(MaxProfitTest, max_profit_postive_elements) { // NOLINT
  int profit = accumulate(positive_elements.begin(), positive_elements.end(), 0);
...
TEST_F(MaxProfitTest, max_profit_integer_limit_elements) { // NOLINT
  long profit = static_cast<long>(numeric_limits<int>::max()) + numeric_limits<int>::max();
